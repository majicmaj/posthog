...
export function createEventSubpipeline<TInput extends EventSubpipelineInput & WithMergeFoldDecision, TContext>(
    ...
    // overhead. Sits after create-event, where the assembled events row exists, and
    // before emit. No retry envelope: a retry would queue the produce again.
    const afterFork = flagEvaluationsService
        ? beforeFork.pipe(createForkFlagEvaluationsStep(outputs, flagEvaluationsService))
        : beforeFork
