...
import { ForkFlagEvaluationsStepInput, createForkFlagEvaluationsStep } from './fork-flag-evaluations-step'
...
const enabledService = () => new FlagEvaluationsService({ mode: 'dual_write', teams: '*', excludedTeams: [] })
const teamExcludedService = () => new FlagEvaluationsService({ mode: 'dual_write', teams: '*', excludedTeams: [7] })

const createStep = (service: FlagEvaluationsService | undefined) => {
...
describe('createForkFlagEvaluationsStep', () => {
    it.each([
        {
            name: 'no service is provided',
            buildService: (): FlagEvaluationsService | undefined => undefined,
    ...
    describe('happy path', () => {
        ...
        it('returns ok(input) unchanged alongside the produce side effect', async () => {
            const { step } = createStep(enabledService())
            const input = createInput()
