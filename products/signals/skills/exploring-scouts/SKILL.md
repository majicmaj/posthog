...
A scout whose config carries a `structured_output_schema` has a third output channel next to reports: schema-validated **measurement records**, recorded as `$scout_structured_output` events in the project (payload keys flattened to `output_<key>` properties, `subject` naming the judged entity) rather than as run-row columns — query the events, or check `metadata.derived.has_structured_output` for a per-run yes/no. See [`references/scout-data-model.md`](references/scout-data-model.md) for the event shape.
