...
/** The query-relevant slice of filter state — the breakdown requests read exactly these four. */
...
export const facetCountsLogic = kea<facetCountsLogicType>([
        ...
        const fetchFacet = async (facet: FacetConfig): Promise<_TracingAttributeBreakdownRowApi[]> => {
            ...
            const response = await tracingSpansAttributeBreakdownCreate(String(values.currentTeamId), {
                query: {
                    ...
                    filterGroup,
                    // Count the population the results list actually shows: traces (matched on
                    // their root span) or every matching span. Same expression the sparkline,
                    // duration histogram and latency heatmap use. Without it the counts are
                    // span-level always, so in Traces view a value carried only by child spans
                    // reads as a non-zero count that selecting it then can't match.
    ...
    selectors({
        ...
        // leaf values — not the `filters` roll-up, which also carries presentation state (orderBy,
        // compareMode, overlay windows) and gets a new identity whenever any of it changes.
        // Subscribing to this keeps a sort toggle from refetching every facet. viewMode is in here
        // because it picks the counted population (see `rootSpans` in fetchFacet), so switching
        // Traces ⇄ Spans has to refetch.
        breakdownScope: [
            (s) => [s.utcDateRange, s.serviceNames, s.queryFilterGroup, s.viewMode],
            (
                utcDateRange: {
                    date_from: string | null | undefined
                    date_to: string | null | undefined
