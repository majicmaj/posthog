...
FORWARDING_CHALLENGE_COOLDOWN_SECONDS = 30
...
class EmailVerifyForwardingView(APIView):
    ...
    def post(self, request: Request, *args, **kwargs) -> Response:
        ...
        cooldown_key = f"customer-email-forwarding-challenge:{setup_id}"
        if not cache.add(cooldown_key, True, timeout=FORWARDING_CHALLENGE_COOLDOWN_SECONDS):
            return Response(
