"""Delivery-contract tests for agent peer messaging (logic/services/peer_messages.py).

Covers the PR's contract list: visibility == send policy, throttles create no rows,
queue cap under the target lock with expiry, idempotent copy-on-send, every
pre-signal failure releasing queue capacity, server-owned context, and the facade's
accepted / target_finished / rejected synchronous semantics.
"""
...
class TestValidateAndPrepare:
    ...
    def test_successful_prepare_creates_accepted_row(self, sender_run, target_run):
        result = self._prepare(sender_run, target_run)
        assert not isinstance(result, PeerMessageRejection)
        message, artifact_ids, resolved_target = result
        assert message.outcome == AgentPeerMessage.Outcome.ACCEPTED
        assert artifact_ids == []
        assert resolved_target.id == target_run.id
        assert message.sender_user_id == sender_run.task.created_by_id
...
class TestArtifactCopyOnSend:
    ...
    def test_copy_re_keys_into_target_prefix_with_deterministic_ids(self, mock_storage, _tag, sender_run, target_run):
        ...
        assert not isinstance(result, PeerMessageRejection)
        message, artifact_ids, _ = result

        expected_id = str(uuid.uuid5(peer_messages.PEER_ARTIFACT_ID_NAMESPACE, f"{message.id}:{entry['id']}"))
        assert artifact_ids == [expected_id]
        ...
        assert message.sender_run_id == sender_run.id
        assert message.target_run_id == target_run.id
        assert message.artifact_ids == [expected_id]
    ...
    def test_copy_retry_produces_exactly_one_manifest_entry(self, mock_storage, _tag, sender_run, target_run):
        ...
        copied = peer_messages._copy_artifacts_to_target(message, sender_artifacts, target_run)
        peer_messages._append_target_manifest_entries(str(target_run.id), copied)

        target_run.refresh_from_db()
        assert [a["id"] for a in target_run.artifacts] == artifact_ids
