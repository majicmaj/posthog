...
For browser QA, target-route readiness is stronger evidence than a fully green local stack. Once the app is reachable, verify the specific route and dependencies the test needs: the changed UI loads, the branch/SHA or another in-diff marker matches the code under test, and any route-critical API returns a usable response. If unrelated phrocs units or `hogli wait` checks are degraded, record them in `run-notes.md` and continue only when they do not explain the target route's behavior. Do not spend the QA run chasing unrelated all-stack health when the target route is already usable.

If the user approves agent-managed startup, prefer the repo's normal background stack: `hogli devbox:start --start-app` for a new or stopped devbox, or `hogli up -d -y` from the checkout that should serve `BASE_URL`. Then narrow readiness checks to the target route, backend/frontend, and any route-specific dependency. Use `hogli dev:intents`, `hogli dev:explain <intent>`, and `hogli dev:list-units <intent>` only as discovery tools when the dependency graph is unclear.
