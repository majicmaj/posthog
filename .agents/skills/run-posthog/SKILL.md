...
For agent-managed QA, prefer the normal detached stack unless the user explicitly asks to tune which services run. On a devbox, `hogli devbox:start --start-app` is the supported path for bringing the app up in the background when a box is created or resumed. If the box is already running and the app is not, run `hogli up -d -y` from `~/posthog` via `devbox:exec`.

```bash
hogli devbox:start --start-app                                      # new or stopped devbox
hogli devbox:exec -- bash -lc 'cd ~/posthog && ./bin/hogli up -d -y' # already-running devbox without the app
hogli up -d -y                                                       # local checkout
```

Use `hogli dev:intents`, `hogli dev:explain <intent>`, and `hogli dev:list-units <intent>` as discovery tools when you need to understand route-specific dependencies. Do not make custom generated configs the default QA startup path.
...
For UI QA, `backend` and `frontend` are hard gates. Add target-specific units when the route needs them, for example `mcp`, `feature-flags`, `nodejs`, `capture`, or `ingestion`. On the default product analytics stack, these `phrocs` units are usually expected to be `ready:true`: `backend`, `frontend`, `nodejs`, `capture`, `ingestion`. Stop here. **Do not chase crashed `migrate-*` units when only `/run` was asked** — the stack is usable for launch, screenshot, and most UI scenes (home, login, settings, feature flags) regardless of migration state.
...
- **`hogli wait` is a coarse all-process signal.** It can fail fast on an unrelated crashed unit, and in some unreachable-phrocs cases its return code is not enough ground truth. Confirm with process-specific `mcp__phrocs__get_process_status`, the `curl` probes, and the target route before deciding the stack is unusable.
